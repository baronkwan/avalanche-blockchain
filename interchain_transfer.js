...
async function main() {
	...
	const ethAddr = ethUtil.Address.fromPrivateKey(
		Buffer.from(keyBuff, "hex")
	).toString("hex");
	...
	setTimeout(async function () {
		...
		console.log(
			`Visit https://cchain.explorer.avax-test.network/address/${ethAddr} for balance details`
		);
		console.log(
			"----------------------------------------------------------------"
		);
...
async function createExport(client, xChain, xKeychain, cKeychain) {
	...
	const assetID = avalanche.BinTools.getInstance().cb58Encode(
		assetInfo.assetID
	);
...
async function createImport(client, cChain, cKeychain, address) {
	...
	// Fetch UTXOs (i.e unspent transaction outputs)
	const { utxos } = await cChain.getUTXOs(
