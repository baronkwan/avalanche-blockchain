...
async function queryPChain(pChain) {
	...
	const ownerBalance = await pChain.getBalance(
...
async function queryXChain(xChain) {
  ...
  const status = await xChain.getTxStatus(
		"2AjbGiRg1KG7FtuJqVEtCzi48n8jpwWdLLYwnBxfFCwMozMLMg"
	);
  console.log("Transaction status:", status);
  
  const balances = await xChain.getAllBalances(
		"X-fuji1h4646056wc84fr7jlmmx7t6u3e348ehwzefl5u"
	);
